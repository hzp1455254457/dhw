...
public class GuideMask :Graphic, ICanvasRaycastFilter
{
    ...
    public void ShowMask(float value, float time, UnityAction unityAction = null,RectTransform rectTransform=null)
    {
        gameObject.SetActive(true);
        enabled = true;
        image.color = new Color(0, 0, 0, 0);
        inner_trans = rectTransform;
        image.DOFade(value, time).SetUpdate(true).onComplete += () => {
            if (unityAction != null)
            {
                unityAction();
            }
    ...
    {
        image.DOFade(0, time).onComplete += () => { gameObject.SetActive(false); if (unityAction != null) { unityAction(); } };
    }
